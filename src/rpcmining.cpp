...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or faicoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        if(fGenerate&&(max((int)pindexBestHeader->nBlockHeight,Checkpoints::GetTotalBlocksEstimate())>chainActive.Height()+3))
...
Value getblocktemplate(const Array& params, bool fHelp)
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        if (!pblocktemplate)
            if(nHeight<=0)
                pblocktemplate = CreateNewBlock(scriptPubKey,chainActive.Height()+1);       
        if (!pblocktemplate)
