...
{
    CAmount nSubsidy = 218750000 * COIN;
    int halvings = (nHeight - 1) / Params().SubsidyHalvingInterval();

    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64)
        return nFees;

    // Subsidy is cut in half every 480 blocks which will occur approximately every 1 day.
    nSubsidy >>= halvings;
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if(nHeight != (int)block.nBlockHeight)
...
bool TestBlockValidity(CValidationState &state, const CBlock& block, CBlockIndex * const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    {
        LogPrintf("main.cpp:TestBlockValidity:pindexPrev is not chainActive.Tip\n");
        return false;
    }
